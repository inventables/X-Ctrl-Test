...
Public Class GrblInterface
    ...
    Private WithEvents PortStatusTimer As Timer 'used to see if the port has closed unexpectedly
    ...
    Private bGotLimitSwitchData As Boolean = False 'is the status reporting limit switch date
    Private LimitSwitch(2) As Boolean

    Private bGotControlSwitchData As Boolean = False 'is the status reporting limit switch date
    Private ControlSwitch(3) As Boolean

    Private bGotProbeSwitchData As Boolean = False 'is the status reporting limit switch date
    Private ProbeSwitch As Boolean
    ...
    Public Event PortClosed() 'the port has closed
    Public Event GotVersion(ByVal Version As String)
    ...
    Public Sub New()
        ...
        PortStatusTimer = New Timer

        PortStatusTimer.Interval = 1000 'once a second
...
#Region "Parsing"
    Private Sub ProcessResponse(Resp As String)
        ...
        bGotLimitSwitchData = False 'change if we get lim data below

        'see what type of response it is
        Select Case Resp.Substring(0, 1)
            Case "G"  'version string like "Grbl 1.0c ['$' for help]"
                Dim Ver As String
                m = Regex.Match(Resp, "Grbl\s(?<Ver>.*?)[\s]")
                If (m.Success) Then
                    Ver = m.Result("${Ver}")
                    RaiseEvent GotVersion(Ver)
                End If
            ...
            Case "<" 'response to a ? command
                ...
                'look for input pin status with regex match
                '   looks like this "Pin:000|1|0111" but last two could be empty "Pin:000||0"  ... it ends with a "," or "]"
                m = Regex.Match(Resp, "Pin:(?<Pin>.*?)[,>]")
                bGotLimitSwitchData = m.Success
                If (m.Success) Then
                    Dim Limits As String = m.Result("${Pin}")
                    Dim Pins() As String = Limits.Split("|")

                    Try
                        If Pins.Count = 3 Then
                            LimitSwitch(AxisLetter.X) = (Pins(0).Substring(2, 1) = "1")
                            LimitSwitch(AxisLetter.Y) = (Pins(0).Substring(1, 1) = "1")
                            LimitSwitch(AxisLetter.Z) = (Pins(0).Substring(0, 1) = "1")

                            bGotLimitSwitchData = m.Success

                            If Pins(1) <> "" Then
                                ProbeSwitch = (Pins(1) = "1") ' Check "Output Window"

                                bGotProbeSwitchData = m.Success
                            End If
                            ...
                            If Pins(2) <> "" Then
                                ...
                                ControlSwitch(ControlPin.Door) = (Pins(2).Substring(3, 1) = "1")
                                ControlSwitch(ControlPin.Reset) = (Pins(2).Substring(2, 1) = "1")
                                ControlSwitch(ControlPin.Feedhold) = (Pins(2).Substring(1, 1) = "1")
                                ControlSwitch(ControlPin.CycleStart) = (Pins(2).Substring(0, 1) = "1")


                                bGotControlSwitchData = m.Success
                            End If
...
#End Region
    ...
    Public Function GetLimitSwitch(ByVal switch As AxisLetter) As Boolean
        If bGotLimitSwitchData Then
            Return LimitSwitch(switch)
        Else
            Throw New Exception("No limit switch status")
        End If
    End Function

    Public Function GetControlSwitch(ByVal switch As ControlPin) As Boolean
        If bGotControlSwitchData Then
            Return ControlSwitch(switch)
        Else
            Throw New Exception("No control switch status")
        End If
    End Function

    Public Function GetProbeSwitch() As Boolean
        If bGotProbeSwitchData Then
            Return ProbeSwitch
        Else
            Throw New Exception("No probe switch status")
        End If
    End Function
    ...
    'check to see if port has closed
    Private Sub PortStatusTimer_tick() Handles PortStatusTimer.Tick
        If Not mSerialPort.IsOpen Then
            PortStatusTimer.Enabled = False
